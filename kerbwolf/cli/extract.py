...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="kw-extract",
        description="Extract Kerberos, SNTP, and NTLM hashes from pcap/pcapng captures.",
...
def main(argv: list[str] | None = None) -> None:
    ...
    # -- header --
    file_desc = ", ".join(args.pcap) if len(args.pcap) <= 3 else f"{len(args.pcap)} file(s)"  # noqa: PLR2004
    print_header(
        "kw-extract",
        [
            ("Attack", "AS-REQ / AS-REP / TGS-REP / Timeroast / NTLM"),
            ("Files", file_desc),
